...
void do_join_cast(int actor_id, int movie_id) {
	...
	*/
	...
	if(!actordb.addMovie(actor_id, movie_id)){
	  cout << "join_cast: Error " << moviedb.getTitle(movie_id) <<
	    " is already in the career of " << actordb.getName(actor_id)
	       << endl;
	}
...
void do_career(int actor_id) {
  //cout << "career: " << endl; 
   // cout << "   actor_id: " << actor_id << endl; 
   /* if(!actordb.find(actor_id)){
       cout << "career: Erro actor id " << actor_id <<
	 "does not exist" << endl;
       return;
       }*/
   actordb.showCareer(actor_id, moviedb);
